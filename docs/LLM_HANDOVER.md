...
**CorridorKey** is a neural-network-based green screen removal tool. It takes an RGB image and a "Coarse Alpha Hint" (often generated by SAM2 or GVM) and produces mathematically perfect, physically unmixed Alpha and Foreground (despilled) color channels.
...
1.  **Model Input/Output is strictly `[0.0, 1.0]` Float Tensors.**
    ...
    *   The predicted Output Foreground (`res['fg']`) is natively sRGB.
    *   The predicted Output Alpha (`res['alpha']`) is inherently Linear.
2.  **EXR Handling (`Processed` Output pass):**
    *   EXRs in professional VFX **must** be stored as Linear float data, premultiplied.
...
*   **Training Code:** We deliberately stripped training-specific logic (like returning coarse logits, `.detach()` gradients, gradient checkpointing) out of the open-source `model_transformer.py` to keep it pristine for inference speed. If the user wants to resume training the Hiera backbone, you may need to re-implement intermediate dictionary returns.
